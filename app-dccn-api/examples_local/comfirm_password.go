...
//	taskClient := taskmgr.NewTaskMgrClient(conn)
	...
	req := &usermgr.ConfirmPasswordRequest{}
	req.Email = "yousong@ankr.com"
	req.ConfirmationCode = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJleHAiOjE1NTA5OTAxODAsImp0aSI6InlvdXNvbmdAYW5rci5jb20iLCJpc3MiOiJhbmtyLm5ldHdvcmsifQ.aBfqwJH8n6erG2tfVcA06LiknY1ybzeyn5XWiGE2KVo"
    req.NewPassword = "1111113c11"


	if _, err := userClient.ConfirmPassword(context.TODO(), req); err != nil {
		//	log.Fatal(err.Error())
		log.Printf("ConfirmPassword have some error : %s \n", err.Error())
	} else {
		log.Printf("ConfirmPassword result no error  ")
	}
