...
//	taskClient := taskmgr.NewTaskMgrClient(conn)
	...
	request := &usermgr.ForgotPasswordRequest{}
	request.Email = "yousong@ankr.network"




	if _, err := userClient.ForgotPassword(context.TODO(), request); err != nil {
		//	log.Fatal(err.Error())
		log.Printf("ForgotPassword have some error : %s \n", err.Error())
	} else {
		log.Printf("ForgotPassword result no error  ")
	}
	...
	//
	//	request := &usermgr.ForgotPasswordRequest{}
	//	request.Email = "yousong.zhang2@gmail.com"
	//
	//
	//
	//
	//	if _, err := userClient.ForgotPassword(tokenContext, request); err != nil {
	//		//	log.Fatal(err.Error())
	//		log.Printf("ForgotPassword have some error : %s \n", err.Error())
	//	} else {
	//		log.Printf("ForgotPassword result no error  ")
